...
2. ロット候補抽出: material_lot のうち、材料が一致し、残量 > 0 のものを抽出
   産地フィルターは材料の分類によって異なる:
     原料(RAW)   : 産地が allowed_origins に含まれるものだけに絞り込む
     添加物(ADDITIVE): 産地の概念が無いため、産地フィルターは行わず、
                       賞味期限のみで選定する(絞り込みをスキップする)
3. 期限順(FEFO)にソート
4. 必要量に達するまで、期限が近い順にロットを引き当てる
   (1ロットで不足する場合は次に期限が近いロットに進む → 複数ロット混合の発生源)
5. 許可産地内で必要量に届かない場合は、実行をブロックし警告表示する
   (産地外での代替は許可しない。添加物は産地の概念自体が無いため対象外)
6. 複数ロット混合が発生する場合、バッチ内の全混合箇所をまとめて
   1回の確認モーダルで表示し、明示的な操作でのみ先に進める
7. 作業員が実測値を入力(システムの計算値はプレースホルダーとして表示)
```

【修正履歴】初版は材料の分類を区別せず一律で産地フィルターをかけていたため、
添加物のallowedOrigins(常に空文字)によって「空リストに含まれるものは無い」という
判定になり、添加物はどのロットも絶対に選定できず、FEFO計算が必ず失敗する不具合が
あった(ManufacturingService.previewFefoAllocationで修正済み、8.15節を参照)。
